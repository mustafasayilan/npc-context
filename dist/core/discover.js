...
import { MANIFESTS, MAX_FILE_BYTES, SKIP_DIRS, SKIP_EXTENSIONS, SOURCE_EXTENSIONS } from "./constants.js";
import { runCommand } from "./platform.js";
import { walkFiles } from "./fs-utils.js";
export function isDependencyTask(task) {
    return /\b(dependenc|package|lock|npm|pnpm|yarn|pip|cargo|nuget|maven|gradle)\w*/i.test(task);
}
export async function isCandidate(root, relativePath, dependencyTask) {
...
export async function discoverFiles(root, task) {
    ...
    const rawFiles = rg.code === 0 || rg.code === 1
        ? rg.stdout.split(/\r?\n/).filter(Boolean)
