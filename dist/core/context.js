...
export async function buildIndex(root, task) {
...
function listOrNone(items, limit = 40) {
    return items.length ? items.slice(0, limit).map((item) => `- ${item}`) : ["- None detected"];
}
function renderContext(task, index, selectedFiles) {
    const lines = [
    ...
    for (const record of selectedFiles) {
        ...
        if (record.routes.length)
            lines.push(`  Routes: ${record.routes.slice(0, 10).join(", ")}`);
        if (record.imports.length)
            lines.push(`  Imports: ${record.imports.slice(0, 10).join(", ")}`);
        if (record.snippet)
            lines.push(`  Snippet: ${record.snippet}`);
    }
    lines.push("", "## Agent Guidance", "- Start with the files, symbols, routes, and git diff paths above.", "- Read additional files only when the task requires it.", "- Do not scan heavy generated, dependency, build, cache, or binary directories.", "- Refresh this context after meaningful edits.", "- NPC bootstrap must not modify application behavior.");
    return `${lines.join("\n")}\n`;
}
export async function createContext(options) {
    const maxFiles = options.maxFiles ?? 20;
    const index = await buildIndex(options.root, options.task);
    const selectedFiles = selectRelevantFiles(Object.values(index.files), taskTerms(options.task), index.git.changedFiles, index.git.untrackedFiles, maxFiles);
