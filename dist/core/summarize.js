...
const SYMBOL_PATTERNS = [
    {
        kind: "type",
        pattern: /^\s*(?:export\s+)?(?:default\s+)?(?:abstract\s+)?(?:class|interface|type|enum|struct|record)\s+([A-Za-z_$][\w$]*)/
    },
    {
        kind: "function",
    ...
    {
        kind: "member",
        pattern: /^\s*(?:public|private|protected|internal|static|async|override|virtual|sealed|\s)+\s*[\w<>,\[\]?]+\s+([A-Za-z_$][\w$]*)\s*\(/
    }
];
...
function roleForPath(relativePath) {
    ...
    if (lower.includes("/scripts/"))
        return "script";
    if (/\.(json|ya?ml|toml|xml)$/.test(lower))
        return "config";
    return "source";
}
export async function summarizeFile(root, relativePath) {
    ...
    for (let index = 0; index < Math.min(lines.length, 5000); index += 1) {
        ...
        for (const pattern of ROUTE_PATTERNS) {
            ...
            for (const match of line.matchAll(pattern)) {
                if (match[1] && routes.length < 30)
                    routes.push(match[1].trim());
            }
        }
        for (const pattern of IMPORT_PATTERNS) {
            const match = pattern.exec(line);
            if (match?.[1] && imports.length < 30)
...
export function detectProjectTypes(files, packageJson) {
