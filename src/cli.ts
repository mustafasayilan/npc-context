...
function optionString(options: Record<string, string | boolean>, name: string, fallback = ""): string {
...
function agentOption(options: Record<string, string | boolean>): AgentSelection {
  const value = optionString(options, "agent", "both");
  if (["codex", "claude", "generic", "both"].includes(value)) return value as AgentSelection;
  throw new Error(`Unsupported agent: ${value}`);
}

function scopeOption(options: Record<string, string | boolean>): InstallScope {
  if (optionBoolean(options, "global")) return "global";
  if (optionBoolean(options, "project")) return "project";
...
async function main(): Promise<number> {
  ...
  if (parsed.command === "context") {
    ...
    const result = await createContext({
      root,
      task,
      agent: agentOption(parsed.options) === "both" ? "generic" : (agentOption(parsed.options) as "codex" | "claude" | "generic"),
    ...
    if (json) {
      ...
      if (result.legacyContextPath) console.log(`Codex compatibility context: ${result.legacyContextPath}`);
  ...
  if (parsed.command === "init" || parsed.command === "install") {
    ...
    if (json) {
      ...
      if (result.gitignoreUpdated) console.log("- Updated .gitignore for NPC helper files");
    }
    return 0;
  }
  ...
  if (parsed.command === "benchmark") {
    ...
    if (!prompt) throw new Error("benchmark requires --prompt or positional prompt text");
